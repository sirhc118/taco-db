...
- **댓글 검증**: 매일 자동 수집 및 D+7 검증으로 포인트 지급
...
### 4. 크론잡
- **댓글 수집 및 검증**: 매일 UTC 06:00에 영상별 배치 처리
- **메트릭 업데이트**: 매일 새벽 2시
...
POST   /api/v1/tasks/:taskId/verify    - 댓글 검증 (초기)
```
...
// → 매일 UTC 06:00 크론잡이 영상의 댓글을 자동 수집
// → D+7 스냅샷과 비교하여 유지된 댓글에 20 NACHO 자동 지급
```
...
```javascript
// src/cron/recheckComments.js - collectAndVerifyComments()
```

**워크플로우:**
1. 활성 캠페인의 모든 영상 조회
2. 각 영상의 댓글 전체 수집 (TikTok 크롤링)
3. 오늘 스냅샷 저장 (`video_comment_snapshots`)
4. 7일 전 스냅샷과 비교
5. 유지된 댓글에 20 NACHO 자동 지급

**예시:**
```
1/1: 댓글 5개 수집 → 스냅샷 저장
1/8: 댓글 수집 → 1/1 댓글 중 4개 유지 확인 → 4명에게 80 NACHO 지급
```

**특징:**
- 영상별 배치 처리로 효율적
- 캠페인 종료된 영상은 자동 제외
- 스냅샷 히스토리 보관 (추후 분석 가능)
...
5. **매일 댓글 수집** (크론잡 - UTC 06:00)
   - 활성 영상의 모든 댓글 자동 수집
   - 스냅샷 저장 (날짜별)
   - 7일 전 스냅샷과 비교
   - 유지된 댓글에 20 NACHO 자동 지급
   - 포인트 거래 기록 저장
